...
## Kode Program (Dengan Penjelasan Per Baris)
  ...
  # SELECTION
    def selection(pop):                            # fungsi untuk memilih parent
        return max(random.sample(pop, 3), key=fitness) # ambil 3 individu acak lalu pilih yang fitness-nya paling tinggi
  ...
  # GA
    def GA():                             # fungsi utama Genetic Algorithm
        ...
        for gen in range(GENERATIONS):    # looping sebanyak 100 generasi
            ...
            pop = sorted(pop, key=fitness, reverse=True)  # urutkan populasi berdasarkan fitness tertinggi

            best = pop[0]                 # ambil individu terbaik
            best_fit = fitness(best)      # hitung fitness terbaik
            avg_fit = sum(fitness(p) for p in pop) / POP_SIZE  # hitung rata-rata fitness
            ...
            if gen % 5 == 0:              # setiap 5 generasi
                print(f"Gen {gen:3d} | Best Value: {best_fit} | Avg: {avg_fit:.2f}") # tampilkan info perkembangan
